...
static errcode_t copy_file_range2(ext2_filsys fs, int fd, ext2_file_t e2_file,
				...
				ext2_file_t e2_file, char *buf, char *zerobuf)
{
	...
	while (data < statbuf->st_size) {
		...
		err = copy_file_range2(fs, fd, e2_file, data_blk, hole_blk, buf,
				      zerobuf);
		if (err)
			return err;
...
#define EXTENT_MAX_COUNT 512
	...
	do {
		...
		for (i = 0, ext = ext_buf; i < fiemap_buf->fm_mapped_extents;
		     i++, ext++) {
			err = copy_file_range2(fs, fd, e2_file, ext->fe_logical,
			   ...
			   ext2_ino_t ino)
{
	...
	err = copy_file_range2(fs, fd, e2_file, 0, statbuf->st_size, buf,
			      zerobuf);
out:
